...
APIDesigner.prototype.init_controllers = function(){
    ...
    this.container.delegate("#define_scopes" ,'click', function(){
        ...
        jagg.post("/site/blocks/item-design/ajax/add.jag",
                {
                   action:"getKeyManagerRolesAndScopes",
                   adminUserName:"admin"
                },
                function (result)
                {
                     var bScopeAdded = false;
                     var bRoleAdded = false;
                     var strHtml = "";
                     if (!result.error)
                     {
                          var objScopesAndRoles = JSON.parse(result.details);
                          
                          if ((null != objScopesAndRoles.scopes) && (0 < objScopesAndRoles.scopes.length))
                          {
                              for (var i=0; i<objScopesAndRoles.scopes.length; i++)
                              {
                                 strHtml += "<option value=\"";
                                 strHtml += objScopesAndRoles.scopes[i] + "\" >" + objScopesAndRoles.scopes[i] + "</option>\n";
                                 bScopeAdded = true;
                              }
                              $('#scopeKey').html(strHtml);
                          }
                          
                          if ((null != objScopesAndRoles.roles) && (0 < objScopesAndRoles.roles.length)) 
                          {
                              strHtml = "";
                              for (var i=0; i<objScopesAndRoles.roles.length; i++)
                              {
                                 strHtml += "<option value=\"";
                                 strHtml += objScopesAndRoles.roles[i] + "\" >" + objScopesAndRoles.roles[i] + "</option>\n";
                                 bRoleAdded = true;
                              }
                              $('#scopeRoles').html(strHtml);
                          }
                     }
                     else
                     {
                          console.log("Define Scopes Error: " + result.error);
                     }
                     if (!bScopeAdded)
                     {
                         strHtml = "Zero scopes were received from the identity provider. Please contact your identity provider administrator.";
                         $('#scopeKeyContainer').html(strHtml);
                         $('#scope_submit').hide();
                     }
                     if (!bRoleAdded)
                     {
                         strHtml = "Zero roles were received from the identity provider. If roles were expected, please contact your identity provider administrator.";
                         $('#scopeRolesContainer').html(strHtml); 
                     }
                     return;
                }, "json");
   ...
   $("#scope_submit").click(function(){
        ...
        if ( $.isArray($("#scopeRoles").val()) )
        {
            if (0 == $("#scopeRoles").val().length)
            {
                $("#scopeRoles").val('');
            }
        }
